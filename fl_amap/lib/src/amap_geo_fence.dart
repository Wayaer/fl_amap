...
class FlAMapGeoFence {
  factory FlAMapGeoFence() => _instance;

  FlAMapGeoFence._();

  static final FlAMapGeoFence _instance = FlAMapGeoFence._();

  static FlAMapGeoFence get instance => _instance;
  ...
  Future<bool> initialize(GeoFenceActivateAction action, [bool allowsBackgroundLocationUpdates = false]) async {
    ...
    final bool? isInit = await _channel.invokeMethod(
        'initialize', {'action': action.index, 'allowsBackgroundLocationUpdates': allowsBackgroundLocationUpdates});
  ...
  Future<AMapGeoFenceAddStatusModel?> addCircle(
    ...
    final result = await _channel.invokeMapMethod('addCircle',
        {'latitude': latLng.latitude, 'longitude': latLng.longitude, 'radius': radius, 'customID': customID});
    return result == null ? null : AMapGeoFenceAddStatusModel.fromMap(result);
  }
        ...
        switch (call.method) {
          case 'updateGeoFence':
            onGeoFenceChanged?.call(call.arguments == null
                ? null
                : AMapGeoFenceStatusModel.fromMap(call.arguments as Map<dynamic, dynamic>));
        }
      });
    }
    return state == true;
...
class AMapPoiModel {
  ...
  Map<String, dynamic> toMap() =>
      {'keyword': keyword, 'poiType': poiType, 'city': city, 'size': size, 'customID': customID};
}
